...
internal class MapMan
{
	...
	public static void DrawMap()
	{
		...
		case 2:
		{
			if (TimeMgr.ZombieTMgr().trackLeft < 8.0)
			{
				float num2 = (float)(TimeMgr.ZombieTMgr().trackLeft / 8.0) + 0.2f;
				DrawMapTrans(ZombieGame.spaceTex, ZombieGame.concreteTex, 1f - num2);
				break;
			}
			DrawJustMap(ZombieGame.spaceTex);
			double num3 = TimeMgr.ZombieTMgr().trackTime;
			if (num3 > 1.0)
			{
				num3 = 1.0;
				if (TimeMgr.ZombieTMgr().trackLeft < 10.0)
				{
					num3 = (TimeMgr.ZombieTMgr().trackLeft - 8.0) / 2.0;
				}
			}
			SpriteTools.BeginAdditive();
			SpriteTools.sprite.Draw(ZombieGame.spaceTex, ScrollMan.GetScreenLoc(mapSize / 2f, 1f), new Rectangle(280, 0, 720, 720), new Color(1f, 1f, 1f, (float)num3), frame / 10f, new Vector2(360f, 360f), 3.3f * ScrollMan.zoom, SpriteEffects.None, 1f);
			SpriteTools.End();
			break;
		}
		case 3:
		...
		case 6:
			...
			{
				float num = (float)(1.0 - TimeMgr.ZombieTMgr().trackLeft / 2.0) + 0.2f;
				DrawMapTrans(ZombieGame.concreteTex, ZombieGame.grassTex, 1f - num);
			}
			else
			{
				DrawJustMap(ZombieGame.grassTex);
