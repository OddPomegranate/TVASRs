...
internal class Menu
{
	...
	private const string AND_SPACE_RAPTOR = "!!!!!!!AND SPACE RAPTOR(S)";
			...
			for (int j = 0; j < 4; j++)
			{
				...
				if (grace <= 0)
				{
					if ((state.Buttons.A == ButtonState.Pressed && pgs[j].Buttons.A == ButtonState.Released) || (state.Buttons.Start == ButtonState.Pressed && pgs[j].Buttons.Start == ButtonState.Released))
					{
                        ...
                        int num = -1;
						for (int k = 0; k < VikingGame.mainPlayerIdx.Length; k++)
						{
						...
						if (num == -1)
						{
							for (int l = 0; l < VikingGame.mainPlayerIdx.Length; l++)
							{
					...
					{
						int num2 = -1;
						for (int m = 0; m < VikingGame.mainPlayerIdx.Length; m++)
						{
	...
	public static void DrawVikingMenu(int rows)
	{
		...
		if (rows > 1)
		{
			Text.DrawString("!!!!!!!AND SPACE RAPTOR(S)", new Vector2(VScroll.screenSize.X / 2f, 280f), 12f, new Color(Rand.GetRandomFloat(0.4f, 1f), Rand.GetRandomFloat(0.4f, 1f), Rand.GetRandomFloat(0.4f, 1f), 1f), Text.Justify.Center);
		}
	}
