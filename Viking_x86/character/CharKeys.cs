...
public class CharKeys
{
	...
	public void Update(int IDX, int ownerID)
	{
		...
		if (autoMode)
		{
			...
			Game1.vgame.charMgr.character[ownerID].lives = 5;
		}
		...
		if (pgs.Buttons.B == ButtonState.Released && state.Buttons.B == ButtonState.Pressed)
		{
			keyCancel = true;
		}
		pgs = state;
