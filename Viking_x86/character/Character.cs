...
public class Character
{
	...
	// Enemy hit-point multiplier by number of active players (index 1-4; index 0
	// unused/never hit). Tune these by feel - e.g. raise index 4 if 4-player
	// games still feel too easy, or lower it if enemies start feeling like
	// damage sponges. Index 2 (2 players) matches this port's original
	// difficulty doubling, so solo/2-player balance is unchanged by default.
	public static readonly float[] enemyHpScaleByPlayers = new float[] { 1f, 1f, 2f, 3f, 4f };
	...
	{
		int count = 0;
		for (int p = 0; p < VikingGame.MONSTER_IDX; p++)
		{
			if (Game1.vgame.charMgr.character[p].exists)
			{
				count++;
			}
		}
	...
	private static float GetLowestActivePlayerY(float fallback)
	{
		bool have = false;
		float best = fallback;
		for (int p = 0; p < VikingGame.MONSTER_IDX; p++)
		{
			Character character = Game1.vgame.charMgr.character[p];
			if (character.exists && (!have || character.loc.Y > best))
			{
				best = character.loc.Y;
				have = true;
			}
		}
		return best;
	...
	// Finds the best AI target among active player slots: nearest by X
	// distance among "valid" candidates (existing, and not mid-respawn when
	// requireNotRespawning is true), falling back to any merely-existing
	// player, falling back to slot 0 if nobody exists at all - matching the
	// original hardcoded 2-player targeting logic, generalized to N players.
	private int GetTargetIndex(bool requireNotRespawning)
	{
		int best = 0;
		...
		for (int p = 0; p < VikingGame.MONSTER_IDX; p++)
		{
			...
			bool valid = !requireNotRespawning || character.respawnFrame <= 0f;
			float d = loc.X - character.loc.X;
			if (d < 0f)
			{
				d = 0f - d;
			}
	...
	{
		if (ID < VikingGame.MONSTER_IDX)
		{
			charKeys.Update(VikingGame.mainPlayerIdx[ID], ID);
			if (nameIn > 0)
			{
				if (nameIn < 4)
				{
		...
		case 2:
		{
			...
			if (team == 1 && loc.Y > GetLowestActivePlayerY(Game1.vgame.charMgr.character[0].loc.Y) + 500f)
			{
				exists = false;
			}
			break;
		...
		case 8:
		{
			...
			for (int k = 0; k < VikingGame.MONSTER_IDX; k++)
			{
		...
		case 6:
		{
			...
			{
				for (int j = 0; j < VikingGame.MONSTER_IDX; j++)
				{
	...
	internal void Init(Vector2 loc, int defID, int face, int team)
	{
		...
		float hpScale = enemyHpScaleByPlayers[Math.Clamp(activePlayers, 1, enemyHpScaleByPlayers.Length - 1)];
		int hpBonus = (int)Math.Round(hpScale - 1f);
		switch (defID)
		{
		case 2:
		case 3:
			if (hpBonus > 0 && Rand.CoinToss(0.5f))
			{
				hp = 1 + hpBonus;
			}
			break;
		case 4:
			hp = (int)Math.Round(10f * hpScale);
			break;
		case 5:
			hp = (int)Math.Round(20f * hpScale);
			break;
		case 7:
		...
		case 8:
			gamma = Rand.GetRandomFloat(2f, 3f);
			hp = (int)Math.Round(15f * hpScale);
			break;
		case 6:
		{
			hp = (int)Math.Round(1f * hpScale);
			angle = 1.57f;
			float num = 0.5f;
			if (face == 0)
			{
	...
	internal void Kill(int killer)
	{
		...
		case 8:
		{
			...
			num2 = 250;
			for (int i = 0; i < VikingGame.MONSTER_IDX; i++)
			{
