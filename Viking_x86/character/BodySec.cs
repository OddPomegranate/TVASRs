...
public class BodySec
{
	...
	internal void Draw(Vector2 loc, float size, int face, Character me, int sec, int flag)
	{
		...
		{
			int num = charDef.GetAnimation(anim).GetKeyFrame(key).frameRef;
			if (num < 0)
			{
				num = 0;
			}
	...
	// Player 2/3/4 (me.ID 1/2/3) each draw from their own pre-colored raptor
	// texture atlas (registered by gfx/*.zsx filename) instead of the shared
	// "raptor" atlas used for Player 1 and any other/default character. Falls
	// back to the character's normal texName if the override texture hasn't
	// been loaded (e.g. its .zsx is missing), so a bad asset never crashes.
	private static readonly string[] playerTexOverride = new string[] { null, "raptor_p2", "raptor_p3", "raptor_p4" };

	private static string GetPlayerTexName(Character me, string defaultTexName)
	{
		int iD = me.ID;
		if (iD >= 0 && iD < playerTexOverride.Length && playerTexOverride[iD] != null && VikingGame.textures.ContainsKey(playerTexOverride[iD]))
		{
			return playerTexOverride[iD];
		}
		return defaultTexName;
	}

	internal Vector2 GetAnchorVec(int sec, Vector2 loc, float size, int face, int frameIdx, int next, Character me)
	{
		CharDef charDef = CharDefMgr.charDef[me.defID];
		string texName = GetPlayerTexName(me, charDef.texName);
		...
		for (int i = 0; i < frame.GetPartArray().Length; i++)
		{
			...
			if (part.idx <= -1 || VikingGame.textures[texName].GetSpriteFlags(part.idx) != sec)
			{
				continue;
			}
			_ = part.rotation;
	...
	internal void Draw(Vector2 loc, float size, int face, int frameIdx, int next, Character me, int sec, bool anchorOnly, int flag)
	{
		...
		for (int i = 0; i < frame.GetPartArray().Length; i++)
		{
			...
			{
			case 1:
				if (VikingGame.textures[texName].GetSpriteFlags(part.idx) != 2 && VikingGame.textures[texName].GetSpriteFlags(part.idx) != 3)
				{
					flag3 = false;
				}
				break;
			case 0:
				if (VikingGame.textures[texName].GetSpriteFlags(part.idx) != 1 && VikingGame.textures[texName].GetSpriteFlags(part.idx) != 3)
				{
					flag3 = false;
				}
				break;
			...
			else
			{
				...
				if (part.idx < 64)
				{
					if (VikingGame.textures.ContainsKey(texName))
					{
						texture2D = VikingGame.textures[texName].texture;
						value = VikingGame.textures[texName].GetSpriteRect(part.idx);
						origin = VikingGame.textures[texName].GetSpriteOrigin(part.idx);
						origin.X -= value.X;
						origin.Y -= value.Y;
						if (!flag4)
						{
				...
				if (texture2D != null)
				{
					...
					{
						position += vector;
						if ((sec != 0 || VikingGame.textures[texName].GetSpriteFlags(part.idx) != 3) && !flag2)
						{
					...
					switch (VikingGame.textures[texName].GetSpriteFlags(part.idx))
					{
					case 3:
				...
				case 1:
				{
					int spriteFlags2 = VikingGame.textures[texName].GetSpriteFlags(part.idx);
					if (spriteFlags2 == 1)
					{
						return;
					}
				...
				case 2:
				{
					int spriteFlags = VikingGame.textures[texName].GetSpriteFlags(part.idx);
					if (spriteFlags == 1)
					{
						flag2 = false;
						flag = 1;
