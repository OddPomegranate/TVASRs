...
public class SpitDrip : BaseParticle
{
	...
	{
		SpriteTools.sprite.Draw(Game1.vgame.spritesTex, VScroll.GetScreenLoc(p.loc, 1f), new Rectangle(576, 256 + Rand.GetRandomInt(0, 3) * 64, 128, 64), GetSpitTint(p.owner, p.r), p.angle + VScroll.angle, new Vector2(0f, 32f), 0.35f * VScroll.zoom * p.size * p.frame, SpriteEffects.None, 1f);
	}
}
